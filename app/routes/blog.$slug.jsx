...
import { getPost } from '~/utils/markdown.server';
...
export default function BlogPost() {
  ...
  const schemaData = {
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `https://trovarci.sh/blog/${post.slug}`,
    },
  };
        ...
        <div className={`container ${styles.inner}`}>
          ...
          <article>
            <header ref={headerRef} className={`${styles.articleHeader} reveal`}>
              {post.category && (
                <Link to={`/blog?category=${encodeURIComponent(post.category)}`} className={styles.category}>
                  {post.category}
                </Link>
              )}
          ...
          <div ref={ctaRef} className={`${styles.bottomCta} reveal`}>
            <p className={styles.bottomCtaHeading}>
              Check your email before you send it
            </p>
            <p className={styles.bottomCtaText}>
              Trovarcis Reach scores your email for deliverability issues
              before it leaves your outbox. Launching June 2026.
            </p>
